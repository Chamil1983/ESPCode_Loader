<%@ Page Language="VB" AutoEventWireup="false" CodeBehind="Default.aspx.vb" Inherits="ArduinoWeb_V1.Default" ValidateRequest="false" %>
<!DOCTYPE html>
...
<head runat="server">
    ...
    <style>
        ...
        * {
        ...
        .binary-tab-pane.active {
            display: block;
        }

        /* Binary upload form styling */
        .binary-row {
            margin-bottom: 10px;
            align-items: center;
        }
        ...
        /* Animation for active selection */
        .binary-row.selected {
            background-color: rgba(0, 82, 204, 0.05);
            padding: 5px;
            border-radius: 4px;
        ...
        @keyframes highlight-row {
            0% { background-color: rgba(0, 82, 204, 0.2); }
            100% { background-color: rgba(0, 82, 204, 0.05); }
        }

        /* Binary button styling */
        #binaryUploadContainer .btn-warning {
            background-color: #FF9800;
            color: white;
            margin-top: 10px;
        }

        #binaryUploadContainer .btn-warning:hover {
            background-color: #F57C00;
        }
        ...
        /* Custom Hardware Board Management */
        ...
        .hardware-board-details strong {
            color: var(--gray-800);
        }
        
        /* Styles for selected hardware board */
        ...
        <div id="mainTab" class="tab-content active">
            ...
            <div class="card">
                ...
                <div class="card-body">
                    <div class="form-row">
                        <div class="form-col">
                            <label for="ddlBoard">ESP32 Processor:</label>
                    ...
                    <!-- Hardware Board Management Section -->
                    <div class="hardware-board-section">
                        <div class="hardware-board-title">
                            <i class="fas fa-microchip"></i> Hardware Board Management
                        </div>
                        
                        <div class="hardware-board-container">
                            <asp:ListBox ID="lstHardwareBoards" runat="server" CssClass="hardware-board-list" 
                                AutoPostBack="true" OnSelectedIndexChanged="lstHardwareBoards_SelectedIndexChanged" />
                            
                            <div class="hardware-board-actions">
                                <asp:Button ID="btnAddBoard" runat="server" Text="Add Board" CssClass="btn btn-sm btn-primary" OnClick="btnAddBoard_Click" />
                                <asp:Button ID="btnUpdateBoard" runat="server" Text="Update" CssClass="btn btn-sm btn-info" OnClick="btnUpdateBoard_Click" />
                                <asp:Button ID="btnDeleteBoard" runat="server" Text="Delete" CssClass="btn btn-sm btn-danger" OnClick="btnDeleteBoard_Click" 
                                    OnClientClick="return confirm('Are you sure you want to delete this hardware board?');" />
                                <asp:Button ID="btnUseBoard" runat="server" Text="Use Board" CssClass="btn btn-sm btn-success" OnClick="btnUseBoard_Click" />
                            </div>
                        </div>
                        
                        <div class="hardware-board-form">
                            <div class="form-row">
                                <div class="form-col">
                                    <label for="txtBoardName">Hardware Board Name:</label>
                                    <asp:TextBox ID="txtBoardName" runat="server" placeholder="Enter hardware board name (e.g., ESP32 DevKit V1)" />
                                </div>
                                <div class="form-col">
                                    <label for="txtBoardDescription">Description (Optional):</label>
                                    <asp:TextBox ID="txtBoardDescription" runat="server" placeholder="Brief description of this hardware board" />
                                </div>
                            </div>
                            
                            <div class="hardware-board-details" id="boardDetailsPanel" runat="server">
                                <div><strong>Processor:</strong> <asp:Label ID="lblBoardProcessor" runat="server" /></div>
                                <div><strong>Partition Scheme:</strong> <asp:Label ID="lblBoardPartition" runat="server" /></div>
                                <div><strong>Last Used:</strong> <asp:Label ID="lblBoardLastUsed" runat="server" /></div>
                            </div>


                            <div class="board-info-row">
    <span class="board-info-label">FQBN:</span>
    <asp:Label ID="lblBoardFQBN" runat="server" CssClass="board-info-value fqbn-preview"></asp:Label>
</div>
<div class="board-info-row">
    <span class="board-info-label">Usage Count:</span>
    <asp:Label ID="lblBoardUsageCount" runat="server" CssClass="board-info-value"></asp:Label>
</div>
<div class="board-info-row">
    <span class="board-info-label">Options:</span>
    <asp:Label ID="lblBoardOptionCount" runat="server" CssClass="board-info-value"></asp:Label>
</div>
                ...
                <div class="card-body">
                    <div class="btn-group">
                        ...
                        <asp:Button ID="btnExportBinary" runat="server" Text="Export Binary" CssClass="btn btn-info" ClientIDMode="Static"
                            OnClick="btnExportBinary_Click" OnClientClick="resetAndStartProgress(); return true;" BackColor="#FFCC66" />
                    ...
                    <!-- Binary Upload Container - UPDATED VERSION WITH SEPARATE BUTTONS -->
                    <div id="binaryUploadContainer" class="form-group" style="margin-top: 16px; padding: 15px; border: 1px dashed #ccc; border-radius: 4px; background-color: #f8f9fa;">
                        <div class="card-header" style="background-color: #f0f0f0; margin-bottom: 15px;">
                            <h3 style="margin: 0; font-size: 16px;"><i class="fas fa-microchip"></i> ESP32 Binary Upload</h3>
                        </div>
                        
                        <!-- Binary upload tabs -->
                        <div class="binary-tabs">
                            <button type="button" id="zipTabBtn" class="binary-tab-btn active" onclick="switchBinaryTab('zipUpload')">
                                <i class="fas fa-file-archive"></i> ZIP Upload
                            </button>
                            <button type="button" id="binTabBtn" class="binary-tab-btn" onclick="switchBinaryTab('binUpload')">
                                <i class="fas fa-microchip"></i> Multi-Binary Upload
                            </button>
                        </div>
                        
                        <div class="binary-tab-content" style="margin-top: 15px;">
                            <!-- ZIP Upload Tab -->
                            <div id="zipUpload" class="binary-tab-pane active">
                                <label for="<%= fuBinaryZip.ClientID %>">Firmware ZIP Package:</label>
                                <div class="file-input-wrapper" style="margin-bottom: 10px;">
                                    <asp:FileUpload ID="fuBinaryZip" runat="server" CssClass="file-input" accept=".zip" ValidateRequestMode="Disabled" OnChange="fuBinaryZip_Change" />
                                    <span class="file-input-label">
                                ...
                                <div class="form-row">
                                    <div class="form-col">
                                        <asp:TextBox ID="txtBinaryPath" runat="server" ReadOnly="true" placeholder="Selected ZIP file path will appear here" CssClass="form-control" />
                                    </div>
                                </div>
                                <div class="tip">
                                    <i class="fas fa-info-circle"></i> Upload a .zip file containing ESP32 binary files to flash to your device.
                                </div>
                                
                                <!-- ZIP Upload Button - Separate from Multi-Binary -->
                                <div class="form-row" style="margin-top: 15px;">
                                    <div class="form-col">
                                        <asp:CheckBox ID="chkVerifyZipUpload" runat="server" Checked="true" Text="Verify upload" />
                                    </div>
                                </div>
                                <div class="form-row" style="margin-top: 10px;">
                                    <div class="form-col">
                                        <asp:Button ID="btnUploadZipBinary" runat="server" Text="Upload ZIP" CssClass="btn btn-warning" 
                                               OnClick="btnUploadZipBinary_Click" OnClientClick="return validateZipUpload();" />
                                    </div>
                                </div>
                            </div>
                            
                            <!-- Binary Upload Tab -->
                            <div id="binUpload" class="binary-tab-pane">
                                <div class="binary-uploads">
                                    <!-- Bootloader Binary (0x1000) -->
                                    <div class="form-row binary-row">
                                        <div class="form-col-label" style="width: 120px;">
                                            <label>Bootloader:</label>
                                        </div>
                                        <div class="form-col" style="width: 100px;">
                                            <asp:TextBox ID="txtBootloaderAddr" runat="server" CssClass="form-control address-input" Text="0x1000" />
                                        </div>
                                        <div class="form-col">
                                            <div class="file-input-wrapper">
                                                <asp:FileUpload ID="fuBootloader" runat="server" CssClass="file-input binary-file-input" accept=".bin" ValidateRequestMode="Disabled" OnChange="fuBootloader_Change" />
                                                <span class="file-input-label">
                                                    <i class="fas fa-microchip"></i> Choose Bootloader
                                                </span>
                                            </div>
                                        </div>
                                        <div class="form-col" style="flex: 2;">
                                            <asp:TextBox ID="txtBootloaderPath" runat="server" ReadOnly="true" placeholder="bootloader.bin" CssClass="form-control binary-path" />
                                        </div>
                                    </div>
                                    
                                    <!-- Partition Table Binary (0x8000) -->
                                    <div class="form-row binary-row">
                                        <div class="form-col-label" style="width: 120px;">
                                            <label>Partition Table:</label>
                                        </div>
                                        <div class="form-col" style="width: 100px;">
                                            <asp:TextBox ID="txtPartitionAddr" runat="server" CssClass="form-control address-input" Text="0x8000" />
                                        </div>
                                        <div class="form-col">
                                            <div class="file-input-wrapper">
                                                <asp:FileUpload ID="fuPartitionTable" runat="server" CssClass="file-input binary-file-input" accept=".bin" ValidateRequestMode="Disabled" OnChange="fuPartitionTable_Change" />
                                                <span class="file-input-label">
                                                    <i class="fas fa-table"></i> Choose Partition
                                                </span>
                                            </div>
                                        </div>
                                        <div class="form-col" style="flex: 2;">
                                            <asp:TextBox ID="txtPartitionPath" runat="server" ReadOnly="true" placeholder="partitions.bin" CssClass="form-control binary-path" />
                                        </div>
                                    </div>
                                    ...
                                    <div class="form-row binary-row">
                                        ...
                                        <div class="form-col" style="width: 100px;">
                                            <asp:TextBox ID="txtAppAddr" runat="server" CssClass="form-control address-input" Text="0x10000" />
                                        </div>
                                        <div class="form-col">
                                            <div class="file-input-wrapper">
                                                <asp:FileUpload ID="fuApplication" runat="server" CssClass="file-input binary-file-input" accept=".bin" ValidateRequestMode="Disabled" OnChange="fuApplication_Change" />
                                                <span class="file-input-label">
                                                    <i class="fas fa-code"></i> Choose App Binary
                                                </span>
                                            </div>
                                        </div>
                                        <div class="form-col" style="flex: 2;">
                                            <asp:TextBox ID="txtAppPath" runat="server" ReadOnly="true" placeholder="app.bin" CssClass="form-control binary-path" />
                                        </div>
                                    </div>
                                    
                                    <!-- SPIFFS/Data Binary (Custom address, often 0x290000) -->
                                    <div class="form-row binary-row">
                                        <div class="form-col-label" style="width: 120px;">
                                            <label>SPIFFS/Data:</label>
                                        </div>
                                        <div class="form-col" style="width: 100px;">
                                            <asp:TextBox ID="txtSpiffsAddr" runat="server" CssClass="form-control address-input" Text="0x290000" />
                                        </div>
                                        <div class="form-col">
                                            <div class="file-input-wrapper">
                                                <asp:FileUpload ID="fuSpiffs" runat="server" CssClass="file-input binary-file-input" accept=".bin" ValidateRequestMode="Disabled" OnChange="fuSpiffs_Change" />
                                                <span class="file-input-label">
                                                    <i class="fas fa-database"></i> Choose SPIFFS
                                                </span>
                                            </div>
                                        </div>
                                        <div class="form-col" style="flex: 2;">
                                            <asp:TextBox ID="txtSpiffsPath" runat="server" ReadOnly="true" placeholder="spiffs.bin" CssClass="form-control binary-path" />
                                        </div>
                                    </div>
                                </div>
                                
                                <div class="tip" style="margin-top: 10px;">
                                    <i class="fas fa-info-circle"></i> Upload individual binary files with specific flash addresses. Standard ESP32 addresses are pre-filled.
                                </div>

                                <!-- Multi-Binary Upload Button - Separate from ZIP -->
                                <div class="form-row" style="margin-top: 15px;">
                                    <div class="form-col">
                                        <asp:CheckBox ID="chkVerifyBinUpload" runat="server" Checked="true" Text="Verify upload" />
                                    </div>
                                </div>
                                <div class="form-row" style="margin-top: 10px;">
                                    <div class="form-col">
                                        <asp:Button ID="btnUploadMultiBinary" runat="server" Text="Upload Binaries" CssClass="btn btn-warning" 
                                               OnClick="btnUploadMultiBinary_Click" OnClientClick="return validateMultiBinaryUpload();" />
                                    </div>
                                </div>
                            </div>
                        </div>
        ...
        <div class="version-info">
            ESP32 Arduino Web Loader v1.5.3 | Last Updated: 2025-08-10 07:46:29 | User: Chamil1983
        </div>
    </div>
    ...
    <!-- Hidden field for selected hardware board ID -->
    <asp:HiddenField ID="hidSelectedBoardId" runat="server" />
    </form>
    ...
    <script type="text/javascript">
        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            // Initialize hardware board selection highlighting
            initHardwareBoardSelection();
        });
        ...
        // -------------------- BINARY UPLOAD FUNCTIONS --------------------
        // Function to switch between binary upload tabs
        function switchBinaryTab(tabId) {
            // Hide all tab panes
            var tabPanes = document.querySelectorAll('.binary-tab-pane');
            for (var i = 0; i < tabPanes.length; i++) {
                tabPanes[i].classList.remove('active');
            }

            // Deactivate all tab buttons
            var tabButtons = document.querySelectorAll('.binary-tab-btn');
            ...
            if (tabId === 'zipUpload') {
                document.getElementById('zipTabBtn').classList.add('active');
            } else {
                document.getElementById('binTabBtn').classList.add('active');
            }

            // Store the active tab in session storage
            sessionStorage.setItem('activeBinaryTab', tabId);

            console.log('Switched to tab: ' + tabId);
        }

        // Function to validate ZIP upload before form submission
        function validateZipUpload() {
            // Check if a ZIP file is selected
            var zipFileInput = document.getElementById('<%= fuBinaryZip.ClientID %>');
            if (!zipFileInput || zipFileInput.files.length === 0) {
                // Check if there's a path already in the text box
                var pathInput = document.getElementById('<%= txtBinaryPath.ClientID %>');
                if (!pathInput || !pathInput.value) {
                    showNotification('Error', 'Please select a ZIP file to upload', 'error');
                    console.log('ZIP file validation failed');
                    return false;
                }
            }

            console.log('ZIP upload validation passed');
            return true;
        }

        // Function to validate multi-binary upload before form submission
        function validateMultiBinaryUpload() {
            // Check if at least one binary file is selected
            var bootloaderInput = document.getElementById('<%= fuBootloader.ClientID %>');
            var partitionInput = document.getElementById('<%= fuPartitionTable.ClientID %>');
            var appInput = document.getElementById('<%= fuApplication.ClientID %>');
            var spiffsInput = document.getElementById('<%= fuSpiffs.ClientID %>');

            var hasFiles = (bootloaderInput && bootloaderInput.files.length > 0) ||
                (partitionInput && partitionInput.files.length > 0) ||
                (appInput && appInput.files.length > 0) ||
                (spiffsInput && spiffsInput.files.length > 0);

            if (!hasFiles) {
                showNotification('Error', 'Please select at least one binary file to upload', 'error');
                console.log('Binary files validation failed');
                return false;
            }

            console.log('Multi-binary upload validation passed');
            return true;
        }

        // Function to set up file input change handlers
        function setupBinaryFileHandlers() {
            // ZIP file selection handler
            var zipInput = document.getElementById('<%= fuBinaryZip.ClientID %>');
            var zipPathDisplay = document.getElementById('<%= txtBinaryPath.ClientID %>');

            if (zipInput && zipPathDisplay) {
                zipInput.addEventListener('change', function () {
                    if (zipInput.files.length > 0) {
                        zipPathDisplay.value = zipInput.files[0].name;
                        console.log('ZIP file selected: ' + zipInput.files[0].name);
                        showNotification('File Selected', 'ZIP file ' + zipInput.files[0].name + ' selected for upload', 'info');
                    } else {
                        zipPathDisplay.value = '';
                    }
                });
            }

            // Binary file selection handlers
            setupBinaryInputHandler('<%= fuBootloader.ClientID %>', '<%= txtBootloaderPath.ClientID %>', 'Bootloader');
            setupBinaryInputHandler('<%= fuPartitionTable.ClientID %>', '<%= txtPartitionPath.ClientID %>', 'Partition Table');
            setupBinaryInputHandler('<%= fuApplication.ClientID %>', '<%= txtAppPath.ClientID %>', 'Application');
            setupBinaryInputHandler('<%= fuSpiffs.ClientID %>', '<%= txtSpiffsPath.ClientID %>', 'SPIFFS/Data');
        }

        // Helper function to set up binary input handlers
        function setupBinaryInputHandler(inputId, pathId, label) {
            var input = document.getElementById(inputId);
            var pathDisplay = document.getElementById(pathId);
            
            if(input && pathDisplay) {
                input.addEventListener('change', function() {
                    if(input.files.length > 0) {
                        pathDisplay.value = input.files[0].name;
                        console.log(label + ' binary file selected: ' + input.files[0].name);
                        
                        // Find and highlight the row
                        var row = input.closest('.binary-row');
                        if(row) {
                        ...
                        showNotification('File Selected', label + ' binary file ' + input.files[0].name + ' selected for upload', 'info');
                    } else {
                        pathDisplay.value = '';
                    }
                });
        ...
        // Function to initialize hardware board selection highlighting
        function initHardwareBoardSelection() {
            var boardList = document.getElementById('<%= lstHardwareBoards.ClientID %>');
            if (boardList) {
                // Add click handler to highlight selected hardware board
                boardList.addEventListener('click', function (e) {
                    // Remove existing highlight from all items
                    Array.from(boardList.options).forEach(function (option) {
                        option.classList.remove('selected-hardware-board');
                    });

                    // Add highlight to clicked item
                    if (boardList.selectedIndex >= 0) {
                        boardList.options[boardList.selectedIndex].classList.add('selected-hardware-board');
                    }
                });

                // Initialize with highlighting if a board is already selected
                if (boardList.selectedIndex >= 0) {
                    boardList.options[boardList.selectedIndex].classList.add('selected-hardware-board');
                }
            }
        ...
        // Initialize binary upload UI on page load
        document.addEventListener('DOMContentLoaded', function() {
            // Initialize binary upload UI
            setupBinaryFileHandlers();
            
            // Restore active tab from session storage
            var activeTab = sessionStorage.getItem('activeBinaryTab');
            if(activeTab) {
                switchBinaryTab(activeTab);
            }
            
            console.log('Binary upload UI initialized');
    ...
    <script type="text/javascript">
        ...
        // Update version info with current date and time
        document.addEventListener('DOMContentLoaded', function () {
            var versionElement = document.querySelector('.version-info');
            if (versionElement) {
                versionElement.innerHTML = 'ESP32 Arduino Web Loader v1.5.3 | Last Updated: 2025-08-10 10:30:42 | User: Chamil1983';
            }
        });
    </script>
</body>
